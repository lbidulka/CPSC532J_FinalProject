...
def main():
    ...
    archive = GridArchive(
        [25, 25],  # 50 bins in each dimension.
    ...
    # Optimizer connects archive and imitter together
    ...
    elite = archive.elite_with_behavior([-0.4, -0.10]) # Choose a behaviour which impacts on left
    for i in range(10):
        simulate(env, elite.sol, seed)
    elite = archive.elite_with_behavior([0, -0.10]) # Choose a behaviour which comes straight down
    for i in range(10):
        simulate(env, elite.sol, seed)
